...
/**
    ...
    Semi Auto PHP ZFS Incremental Synchronisation Tool between local pools
**/

$config = array();
$config["snapshot"] = false; // DO SNAPSHOTING
//$config["snapshot_datasets"][] = "zroot";
$config["snapshot_datasets"][] = "zroot/storage/vm/datalogger";
$config["snapshot_datasets"][] = "zroot/storage/vm/fileserver";
$config["snapshot_datasets"][] = "zroot/storage/vm/mn";
$config["snapshot_datasets"][] = "zroot/storage/vm/old_sys";
$config["snapshot_datasets"][] = "zroot/storage/vm/openerp";
//$config["snapshot_datasets"][] = "zroot/usr";
//$config["snapshot_datasets"][] = "zroot/usr/ports";
//$config["snapshot_datasets"][] = "zroot/var";

$config["keep_snapshots"] = 14; //MAXIMUM SNAPSHOTS
$config["snapshot_interval"] = 60*24; //minute //SNAPSHOT INTERVAL

$config["sync"] = true; // DO SYNCING
$config["sync_datasets"][] = "zroot/devel:zbackup/devel";
$config["sync_datasets"][] = "zroot/ext-images:zbackup/ext-images";
$config["sync_datasets"][] = "zroot/documents:zbackup/documents";
$config["sync_datasets"][] = "zroot/backup:zbackup/backup";
$config["sync_datasets"][] = "zroot/photoshop:zbackup/photoshop";
$config["sync_interval"] = 0; //minute //SYNC INTERVAL

$config["version"] = 1;

$config["mutex_file"] = "/zroot/zfsrep.lock";
$config["data_file"] = "/zroot/zfsrep.data";
$config["log_file"] = "/zroot/zfsrep.log";
...
var $data;
	...
	{
		$arr = $this->snapshot_list($src,"bck");
		if(count($arr))
		{
			logger("Syncing already running, so update incrementaly ..");
			$ret = $this->snapshot_create($src,"bck");
			if(!$ret)
			{
	...
	{
		global $config;
		$temp = shell_exec("zfs send \"".$src."\" | zfs recv -vFd ".$dst." 2>&1");
		if(strstr($temp,"received"))
		{
			...
			logger("Rename on DST: ".$temp." to: ".$name[0]."@latest_bck");
			logger(shell_exec("zfs rename ".$temp." ".$name[0]."@latest_bck 2>&1"));
			return true;
		}
		else
		{
	...
	{
	global $config;
		$temp = shell_exec("zfs send -i \"".$src."\" \"".$src2."\" | zfs recv -vFd ".$dst." 2>&1");
		if(strstr($temp,"received"))
		{
			logger("SUCCESS: ".$temp);
			...
			$name = explode("@",$temp);
			logger("Delete on DST: ".$name[0]."@latest_bck");
			
			logger(shell_exec("zfs destroy ".$name[0]."@latest_bck 2>&1"));
